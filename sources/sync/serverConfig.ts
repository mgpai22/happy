...
export function getServerInfo(): { hostname: string; port?: number; isCustom: boolean } {
	...
	try {
		...
		const port = parsed.port ? Number.parseInt(parsed.port) : undefined;
		return {
...
export function getFusionServerUrl(): string {
	return (
		serverConfigStorage.getString(FUSION_SERVER_KEY) ||
...
export function getOpenCodeServerUrl(): string {
	return (
		serverConfigStorage.getString(OPENCODE_SERVER_KEY) ||
...
const OPENCODE_PASSWORD_KEY = 'opencode-password';
const DEFAULT_OPENCODE_PASSWORD = 'zISgrhgNqILaT87kOluKF9beBRlRve';

export function getOpenCodePassword(): string {
	return (
		serverConfigStorage.getString(OPENCODE_PASSWORD_KEY) ||
		process.env.EXPO_PUBLIC_OPENCODE_PASSWORD ||
...
export function setOpenCodePassword(password: string | null): void {
	if (password && password.trim()) {
		serverConfigStorage.set(OPENCODE_PASSWORD_KEY, password.trim());
	} else {
		serverConfigStorage.delete(OPENCODE_PASSWORD_KEY);
	}
}
